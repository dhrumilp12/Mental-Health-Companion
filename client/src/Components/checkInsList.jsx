...
function CheckInsList() {
            ...
            if (
                ...
                const formattedCheckIns = response.data.map((checkIn) => ({
                    ...
                    check_in_time: new Date(
                        checkIn.check_in_time.$date
                    ).toLocaleString(), // Convert date from MongoDB format to a readable string
                }));
    ...
    const handleDeleteCheckIn = async () => {
        if (selectedCheckIn) {
            try {
                await apiServerAxios.delete(
                    `/check-in/${selectedCheckIn._id}`,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`, // Ensure the Authorization header is set
                        },
    ...
    return (
        ...
        >
            ...
            <Dialog open={dialogOpen} onClose={handleCloseDialog}>
                ...
                <DialogContent>
                    <Typography component="div">
                        ...
                        <Typography variant="body1">
                            <strong>Status:</strong> {selectedCheckIn?.status}
                        </Typography>
                        <Typography variant="body1">
                            <strong>Notify:</strong>{" "}
                            {selectedCheckIn?.notify ? "Yes" : "No"}
                        </Typography>
            ...
            <Dialog open={deleteConfirmOpen} onClose={handleCloseDialog}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Are you sure you want to delete this check-in? This
                        action cannot be undone.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleDeleteCheckIn} color="error">
                        Delete
                    </Button>
