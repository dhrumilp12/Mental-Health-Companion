      ...
      if (
        ...
        const formattedCheckIns = response.data.map((checkIn) => ({
          ...
          check_in_time: new Date(checkIn.check_in_time.$date).toLocaleString(
            "en-US",
            {
              timeZone: "UTC", // Ensure you're using the correct timezone
        ...
        <DialogContent>
          <Typography component="div">
            ...
            <Typography variant="body1">
              <strong>Time:</strong> {selectedCheckIn?.check_in_time}
            </Typography>
            <Typography variant="body1">
              <strong>Frequency:</strong> {selectedCheckIn?.frequency}
            </Typography>
            <Typography variant="body1">
              <strong>Status:</strong> {selectedCheckIn?.status}
            </Typography>
            <Typography variant="body1">
              <strong>Notify:</strong> {selectedCheckIn?.notify ? "Yes" : "No"}
            </Typography>
            {/* You can add more details here as per your data model */}
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleUpdateRedirect} startIcon={<EditIcon />}>
            Update
          </Button>
          <Button
